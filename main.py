...
class LibraryManagementSystem:
    def __init__(self, root):
        ...
        # to style the label
        label_title = Label(self.root, text="LIBRARY MANAGEMENT SYSTEM", bg="powder blue", fg="green", bd=10,
                            relief=RIDGE, font=("Helvetica", 35, "italic"), padx=2, pady=6)
        ...
        frame.place(x=0, y=115, width=1530, height=400)
        ...
        label_member = Label(DataFrameLeft, text="Member Type : ", font=("Helvetica", 10), padx=5, pady=5,
                             bg='powder blue')
        ...
        label_ID = Label(DataFrameLeft, text="ID Number : ", font=("Helvetica", 10), padx=5, pady=5, bg='powder blue')
        label_ID.grid(row=1, column=0, sticky=W)

        txt_ID = Entry(DataFrameLeft, font=('Google Sans', 10), textvariable=self.member_id_var, width=29)
        txt_ID.grid(row=1, column=1)

        # ---------------Title--------------------

        # labels for the title details
        label_tit = Label(DataFrameLeft, text="Book Type : ", font=("Helvetica", 10), padx=5, pady=5, bg='powder blue')
        label_tit.grid(row=2, column=0, sticky=W)

        txt_tit = Entry(DataFrameLeft, font=('Google Sans', 10), textvariable=self.title_var, width=29)
        txt_tit.grid(row=2, column=1)

        # ---------------First Name--------------------

        # labels for the first name details
        label_first_name = Label(DataFrameLeft, text="First Name : ", font=("Helvetica", 10), padx=5, pady=5,
                                 bg='powder blue')
        label_first_name.grid(row=3, column=0, sticky=W)

        txt_first_name = Entry(DataFrameLeft, font=('Google Sans', 10), textvariable=self.first_name_var, width=29)
        txt_first_name.grid(row=3, column=1)

        # ---------------Last Name--------------------
        # labels for the last name details
        label_last_name = Label(DataFrameLeft, text="Last Name : ", font=("Helvetica", 10), padx=5, pady=5,
                                bg='powder blue')
        label_last_name.grid(row=4, column=0, sticky=W)

        txt_last_name = Entry(DataFrameLeft, font=('Google Sans', 10), textvariable=self.last_name_var, width=29)
        txt_last_name.grid(row=4, column=1)

        # ---------------Mobile Number--------------------
        # labels for the Mobile number details
        label_mobile_number = Label(DataFrameLeft, text="Mobile Number : ", font=("Helvetica", 10), padx=5, pady=5,
                                    bg='powder blue')
        label_mobile_number.grid(row=5, column=0, sticky=W)

        txt_mobile_number = Entry(DataFrameLeft, font=('Google Sans', 10), textvariable=self.mobile_number_var,
                                  width=29)
        txt_mobile_number.grid(row=5, column=1)

        # ---------------Address Line 1--------------------
        # labels for the address details
        label_address = Label(DataFrameLeft, text="Address : ", font=("Helvetica", 10), padx=5, pady=5,
                              bg='powder blue')
        label_address.grid(row=6, column=0, sticky=W)

        txt_address = Entry(DataFrameLeft, font=('Google Sans', 10), textvariable=self.address_var, width=29)
        txt_address.grid(row=6, column=1)

        # ---------------Book Name--------------------
        # labels for the bookName details
        label_bookName = Label(DataFrameLeft, text="Book Name : ", font=("Helvetica", 10), padx=5, pady=5,
                               bg='powder blue')
        label_bookName.grid(row=7, column=0, sticky=W)

        txt_bookName = Entry(DataFrameLeft, font=('Google Sans', 10), textvariable=self.book_name_var, width=29)
        txt_bookName.grid(row=7, column=1)

        # ---------------Book Author--------------------
        # labels for the bookID detail
        label_bookID = Label(DataFrameLeft, text="Book Author : ", font=("Helvetica", 10), padx=5, pady=5,
                             bg='powder blue')
        label_bookID.grid(row=8, column=0, sticky=W)

        txt_bookID = Entry(DataFrameLeft, font=('Google Sans', 10), textvariable=self.book_author_var, width=29)
        txt_bookID.grid(row=8, column=1)

        # ---------------Book ID--------------------
        # labels for the bookID details
        label_bookID = Label(DataFrameLeft, text="Book ID : ", font=("Helvetica", 10), padx=5, pady=5, bg='powder blue')
        label_bookID.grid(row=9, column=0, sticky=W)

        txt_bookID = Entry(DataFrameLeft, font=('Google Sans', 10), textvariable=self.book_id_var, width=29)
        txt_bookID.grid(row=9, column=1)

        # ---------------Issue Date--------------------

        # labels for the issueDate details
        label_issue_date = Label(DataFrameLeft, text="Issue Date : ", font=("Helvetica", 10), padx=5, pady=5,
                                 bg='powder blue')
        label_issue_date.grid(row=0, column=2, sticky=W)
        txt_issue_date = Entry(DataFrameLeft, font=("Helvetica", 10), textvariable=self.issue_date_var, width=29)
        txt_issue_date.grid(row=0, column=3)

        # ---------------Due Date--------------------
        # labels for the dueDate details
        label_due_date = Label(DataFrameLeft, text="Due Date : ", font=("Helvetica", 10), padx=5, pady=5,
                               bg='powder blue')
        label_due_date.grid(row=1, column=2, sticky=W)

        txt_due_date = Entry(DataFrameLeft,font=("Helvetica", 10), textvariable=self.due_date_var,
                             width=29)
        txt_due_date.grid(row=1, column=3)

        # ---------------Late Return Fine--------------------
        # labels for the late return fine  details
        label_lateReturn = Label(DataFrameLeft, text="Late Return Fine : ", font=("Helvetica", 10), padx=5, pady=5,
                                 bg='powder blue')
        label_lateReturn.grid(row=2, column=2, sticky=W)

        txt_lateReturn = Entry(DataFrameLeft, font=('Google Sans', 10), textvariable=self.late_fine_var, width=29)
        txt_lateReturn.grid(row=2, column=3)

        # -----------------------------------
        # labels for the Actual Price of the book details
        label_book_price = Label(DataFrameLeft, text="Book Price : ", font=("Helvetica", 10), padx=5, pady=5,
                                 bg='powder blue')
        label_book_price.grid(row=3, column=2, sticky=W)

        txt_book_price = Entry(DataFrameLeft, font=('Google Sans', 10), textvariable=self.book_price_var, width=29)
        txt_book_price.grid(row=3, column=3)

        # --------------------------------------------DataFrame Right--------------------------------------------------------------
        # to have another frame in the right
        DataFrameRight = LabelFrame(frame, text="Book Details", bg="powder blue", fg="green", bd=4, relief=RIDGE,
                                    font=("Helvetica", 25, "italic"))
        # for placing the frame with co-ordinates as given
        DataFrameRight.place(x=870, y=5, width=580, height=350)

        self.txtBox = Text(DataFrameRight, font=("Helvetica", 10), width=50, height=16, padx=2)
        ...
        def selectbook(event=''):
            ...
            print("hello : ", value)
            try:
                conn = mysql.connector.connect(host="localhost", username="root", password="Krish2001@",
                                               database="library")
        ...
        listBox = Listbox(DataFrameRight, font=("Helvetica", 10), width=20, height=18)
        ...
        # --------------------------------------------Button Frame---------------------------------------------------------------
        #
        frameButton = Frame(self.root, bd=4, relief=RIDGE, padx=340, pady=15, bg="powder blue")
        frameButton.place(x=0, y=515, width=1530, height=70)

        # adding buttons in the frameButton
        btnAddData = Button(frameButton, command=self.add_data, text='Add Data', font=('Google Sans', 10), padx=3,
                            pady=3, width=23, bg="blue", fg="white")
        btnAddData.grid(row=0, column=0)

        btnShowData = Button(frameButton, text='Show Data', command = self.show_data,font=('Google Sans', 10), padx=3, pady=3, width=23,
                             bg="blue", fg="white")
        btnShowData.grid(row=0, column=2)


        btnResetData = Button(frameButton, text='Reset', command = self.reset ,font=('Google Sans', 10), padx=3, pady=3, width=23, bg="blue",
                              fg="white")
        btnResetData.grid(row=0, column=4)

        btnExit = Button(frameButton, text='Exit', command = self.exit_func,font=('Google Sans', 10), padx=3, pady=3, width=23, bg="blue",
                         fg="white")
        btnExit.grid(row=0, column=6)
        #  --------------------------------------------Database Frame--------------------------------------------------------------

        # Creating a frame in the lower part
        frameDatabase = Frame(self.root, bd=4, relief=RIDGE, padx=20, pady=5, bg="powder blue")
        frameDatabase.place(x=0, y=585, width=1530, height=210)

        # Creating a frame inside the above mentioned frame
        table_frame = Frame(frameDatabase, bd=4, relief=RIDGE, padx=20, bg="powder blue")
